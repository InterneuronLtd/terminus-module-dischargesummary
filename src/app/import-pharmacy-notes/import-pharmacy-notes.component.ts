...
@Component({
  selector: 'app-import-pharmacy-notes',
  templateUrl: './import-pharmacy-notes.component.html',
  styleUrls: ['./import-pharmacy-notes.component.css']
})
export class ImportPharmacyNotesComponent implements OnInit, OnDestroy {
  ...
  constructor(public subjects: SubjectsService,private activeModal: NgbActiveModal, public appService: AppService, public apiRequest: ApirequestService) { }
      ...
      this.apiRequest.postRequest(this.appService.baseURI + '/GetListByPost/core/dischargesummary',this.createClinicalSummaryNotesFilter())
        .subscribe((response) => {
          if(response)
          {
            ...
            this.notesData[0].pharmacynotes = this.pharmacyNotes;
            // this.subjects.importPharmacyNotes.next(this.notesData[0].pharmacynotes);
            this.notesData[0].pharmacynotescreatedby = this.appService.loggedInUserName;
            this.notesData[0].pharmacynotestimestamp = this.appService.getDateTimeinISOFormat(new Date());
      ...
      this.apiRequest.postRequest(this.appService.baseURI + '/PostObject?synapsenamespace=core&synapseentityname=dischargesummary', this.notesData[0])
        .subscribe((response) => {
          if(response)
          {
            ...
            this.subjects.importPharmacyNotes.next('');
            this.dismiss();
          }
        })
      )
