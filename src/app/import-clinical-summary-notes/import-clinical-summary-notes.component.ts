...
@Component({
  selector: 'app-import-clinical-summary-notes',
  templateUrl: './import-clinical-summary-notes.component.html',
  styleUrls: ['./import-clinical-summary-notes.component.css']
})
export class ImportClinicalSummaryNotesComponent implements OnInit, OnDestroy {
  ...
  importClinicalNotes(){
    this.subscriptions.add(
      this.apiRequest.getRequest(this.appService.carerecorduri + '/ClinicalSummary/GetClinicalSummaryNote/' + this.appService.personId)
        .subscribe((response) => {
          if(response == '[]')
          {
      ...
      this.apiRequest.postRequest(this.appService.baseURI + '/GetListByPost/core/dischargesummary',this.createClinicalSummaryNotesFilter())
        .subscribe((response) => {
          if(response)
          {
            ...
            this.notesData[0].clinicalsummarynotes = this.clinicalSummaryNotes;
            // this.subjects.importClinicalSummaryNotes.next(this.notesData[0].clinicalsummarynotes);
            this.notesData[0].clinicalsummarynotescreatedby = this.appService.loggedInUserName;
            this.notesData[0].clinicalsummarynotestimestamp = this.appService.getDateTimeinISOFormat(new Date());
    ...
    this.subscriptions.add(
      this.apiRequest.postRequest(this.appService.baseURI + '/PostObject?synapsenamespace=core&synapseentityname=dischargesummary', this.notesData[0])
        .subscribe((response) => {
          if(response)
          {
            if(response[0].clinicalsummarynotes == '' || response[0].clinicalsummarynotes == null)
            {
            ...
            this.subjects.importClinicalSummaryNotes.next('');
            this.dismiss();
          }
        })
      )
